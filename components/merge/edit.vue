  ...
  methods: {
    ...
    async next () {
      ...
      this.merge.list_mapping = this.merge.list_mapping.map(item => {
        const newCol = []
        item.listCol.forEach((col, i) => {
          this.tables.forEach(table => {
            if (col.includes(table.table) === true) {
              return newCol.push(col.split('.').pop())
            }
          })
          newCol.push('')
        })
      ...
      // this.merge.list_mapping = this.merge.list_mapping.map(item => {
      //   const newCol = item.listCol.forEach(col => {
      //     this.tables.forEach(table => {
      //       console.log(col.includes(table.table))
      //       if (col.includes(table.table)) {
