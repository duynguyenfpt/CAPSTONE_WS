    ...
    <div>
          ...
          >
            ...
            <tbody>
              <tr v-for="(table, k) in tables" :key="k">
                ...
                <td v-for="(tb, key) in tbs" :key="key">
                  <v-select
                    class="select-sm"
                    :reduce="(text) => text.value"
                    label="text"
                    :options="opsColName"
                    v-model="$data['table.' + tb.text]"
                    size="sm"
      ...
      <b-row class="pt-2">
        ...
        <b-col sm="8" class="text-center">
          <b-btn @click="next" size="sm" variant="primary" :disabled="active===1">Next step</b-btn>
          <b-btn
            size="sm"
            variant="primary"
...
import { getAllTableByDb, getTableDetail, getColumnByTable } from '@/service/table.service'
import Vue from 'vue'
...
export default {
  ...
  methods: {
    ...
    next () {
      ...
      this.opsColName = columns
      console.log(columns.covid_19_data)
      console.log(this.opsColName.covid_19_data)
    }
  }
}
</script>
